...
3. Import `fixtures/CD2.FEL.json` and `fixtures/CD2.MEME.json` together. Both remain in the analysis list; selecting either shows its native track and source-specific inspector. FEL codon 11 illustrates supported purifying selection at p ≤ 0.05; MEME codon 43 illustrates episodic diversification.
   When one FEL and one MEME Datamonkey export agree on the full comparison metadata contract, the Dash app also adds and selects a **metadata-matched** two-track view. The individual method views remain available.
...
The Dash app can co-display exactly one FEL and one MEME result when both have complete Datamonkey export metadata and agree on export filename, native input filename, sequence/site counts, the entire coordinate coverage array, named leaf taxa, and tested/background scope for every leaf. This is labeled **metadata-matched**, not alignment-verified. Both method-native tracks and inspectors remain separate; their values are never joined into a score or branch–site claim. Individual views remain available.

The JSON still lacks an alignment-content hash or embedded original alignment. Consequently, matching only a filename, length, taxa, or tree is insufficient, and files with incomplete or differing comparison metadata remain separate. Method-specific input trees are preserved and may differ; they are not used to claim alignment identity. Verified native linking remains a Priority 2 limitation requiring analysis-time content hashes and explicit coordinate provenance.
