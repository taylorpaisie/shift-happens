        ...
        return html.Div([
            ...
            html.Main([
                html.Section([html.Div([html.P("EVOLUTION, IN CONTEXT", className="eyebrow"), html.H2(["Different methods.", html.Br(), "Distinct evidence."]), html.P("Explore conservation and diversification while keeping each statistical question in view.")]),
                              html.Div([dcc.Upload(id="upload", children=html.Button("＋ Import HyPhy / Datamonkey JSON", className="primary"), multiple=True, accept=".json,application/json"), html.P(f"FEL 2.00 / 2.6 · MEME 2.00 / 2.1.1 · {settings.max_file_mib} MiB/file"), html.P("Uploads are sent to this hosted server for processing. Use local mode for data that must stay on your computer." if settings.hosted else "Files are processed by your local Python server."), html.Button("Open synthetic example →", id="demo", className="text-button"), html.Details([html.Summary("Importing Datamonkey results"), html.P("On a completed FEL or MEME results page, download the full results JSON and upload it here. CSV tables, job-status JSON, and saved webpages are not complete analysis results."), html.P("Only the method versions listed above are validated. Newer versions need a representative fixture; do not change a file’s version field.")])], className="import-box")], className="intro"),
